...
module.exports = {
    getIntersection(sentence1, sentence2) {
        let k = sentence1.split(' ')
        let v = sentence2.split(' ')

        if ( (k.length + v.length) != 0 ) {
            let length = (k.length + v.length) / 2
            let intersect = k.filter(l => v.includes(l))
            
            return intersect.splice(0, length).length
        } else return 0
    },

    formatSentece(sentence) {
    ...
    bestSentence(paragraph, dict) {
        let Sentence = Tokenizer.sentences(paragraph, { newline_boundaries: true }) || []
        
        if ( Sentence.length < 2 ) return ''

        let bS = '' // Best Sentence
        let mV = 0 // Max Value

        for ( let k in Sentence ) {
            let v = this.formatSentece(Sentence[k])
    ...
    sentenceRank(content) {
        let Dict = new Object()
        let Sentences = Tokenizer.sentences(content, { newline_boundaries: true }) || []
        
        let n = Sentences.length
        let r = new Array(n).fill(0).map((_, i) => i) // range.
        let v = new Array(n).fill(new Array(n).fill(0), 0, n)
        
        for ( let i in r ) {
            for ( let j in r ) {
                v[i][j] = this.getIntersection(Sentences[i], Sentences[j])
            }
        }
        ...
        for ( let i in r ) {
            ...
            Dict[this.formatSentece(Sentences[i])] = Score
        }

        return Dict
    },

    getSummary(title, content, dict) {
        ...
        for ( let p in Paragraph ) {
            let Sentence = this.bestSentence(Paragraph[p], dict).trim()
